...
async def handle_generic_file_message(
    message: Any,
    *,
    route_label: str,
    log_incoming_message_fn: Callable[[Any, str], None],
    ...
    log_incoming_message_fn(message, route_label)
    logger.info(
        "Entering handle_%s_message: chat=%s thread=%s message=%s",
        route_label,
        message.chat.id,
        thread_id_fn(message),
        message.message_id,
    ...
    try:
        ...
        logger.exception("Unhandled exception in handle_%s_message", route_label)
        metrics.MESSAGES_TOTAL.labels(status="error").inc()
        scope_key = scope_key_from_message_fn(message)
        record_error_fn(scope_key)
        reply_markup = build_rollback_suggestion_markup_fn(
