...
async def test_execute_task_realigns_provider_scope_before_usage_limit_fallback(monkeypatch) -> None:
    ...
    providers = [
        SimpleNamespace(name="codex", cli="codex", model=None, resume_arg=None),
        SimpleNamespace(name="codex2", cli="codex2", model=None, resume_arg=None),
        SimpleNamespace(name="codex3", cli="codex3", model=None, resume_arg=None),
    ]
    scope_state = {"123:main": 2}
    ...
    assert set_calls == [("123:main", "codex2"), ("123:main", "codex3")]
    assert attempts == [
        ("codex2", {"FAKE_PROVIDER_ENV": "1"}),
        ("codex3", {"FAKE_PROVIDER_ENV": "1"}),
    ]
    assert task.provider_cli == "codex3"
    assert task.status == TaskStatus.COMPLETED
    assert task.response == "ok-after-realignment"


@pytest.mark.asyncio
async def test_execute_task_cycles_through_codex_family_before_non_codex_fallback(monkeypatch) -> None:
    bot = AsyncMock()
    providers = [
        SimpleNamespace(name="codex", cli="codex", model=None, resume_arg=None),
        SimpleNamespace(name="codex2", cli="codex2", model=None, resume_arg=None),
        SimpleNamespace(name="codex3", cli="codex3", model=None, resume_arg=None),
        SimpleNamespace(name="claude", cli="claude", model=None, resume_arg=None),
    ]
    scope_state = {"123:main": 2}
    set_calls: list[tuple[str, str]] = []
    ...
    async def fake_stream_codex_message(**kwargs):
        ...
        if len(attempts) < 3:
            yield bridge.StreamEvent(
                ...
                response=bridge.ClaudeResponse(
                    text="You've hit your usage limit.",
                    session_id=f"sess-{len(attempts)}",
                    is_error=True,
                    cost_usd=0.0,
                ),
            )
    ...
    assert set_calls == [("123:main", "codex"), ("123:main", "codex2")]
    assert attempts == ["codex3", "codex", "codex2"]
    assert task.provider_cli == "codex2"
    assert task.status == TaskStatus.COMPLETED
    assert task.response == "ok-after-cycling"
