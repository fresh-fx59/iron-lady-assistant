...
class DigestLedger:
    ...
    def _revert_to_approved(self, date_key: str) -> None:
        with self._connect() as con:
            con.execute(
...
def publish_next(
    ...
    *,
    ...
    if effective_dry:
        ...
        ledger._revert_to_approved(date_key)  # noqa: SLF001 — same-module state authority
    ...
    try:
        ...
        ledger.mark_failed(date_key, str(exc))
        logger.error("aggregator publish: send failed for %s: %s", date_key, exc)
        return {"status": "failed", "date_key": date_key, "error": str(exc)}
